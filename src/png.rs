...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        if let Some(index) = self.data.iter().position(|x| *x.chunk_type() == tmp_chunk_type) {
            return Ok(self.data.remove(index));
        }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        self.header
            .clone()
            .into_iter()
            .chain(self.data.iter().flat_map(|chunk| chunk.as_bytes()))
            .collect()
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Png> {
        ...
        let mut tmp_header: [u8; 8] = [0; 8];
        let mut tmp_chunks: Vec<Chunk> = Vec::new();

        reader.read_exact(&mut tmp_header)?;
        
        if tmp_header != Png::STANDARD_HEADER {
            return Err("Invalid Header".into())
        }

        let mut tmp_buffer = [0u8; 4];
        while reader.read(&mut tmp_buffer).unwrap_or(0) > 0 {
            let chunk_length = u32::from_be_bytes(tmp_buffer);
            reader.seek(SeekFrom::Current(-4))?;
            let tmp_chunk = Chunk::try_from(&value[reader.position() as usize..(reader.position()+chunk_length as u64 + 12) as usize])?;
            tmp_chunks.push(tmp_chunk);
            reader.seek(SeekFrom::Current(chunk_length as i64 + 12i64))?;
        }
